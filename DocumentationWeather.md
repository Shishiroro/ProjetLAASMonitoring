...
## Comportement global de la météo

Le moteur décide quoi injecter selon `precip_rate` et `cloud_type` :

| Cas | Condition | Résultat |
|-----|-----------|----------|
| **Pluie seule** | `precip_rate > 0` ET `cloud_type = -1` | Cumulonimbus forcé auto (couverture 1.0) |
| **Nuages seuls** | `cloud_type >= 0` ET `precip_rate = 0` | Nuages manuels, pas de précipitation |
| **Pluie + nuages** | `precip_rate > 0` ET `cloud_type >= 0` | Nuages manuels utilisés comme source |
| **Rien** | `precip_rate = 0` ET `cloud_type = -1` | Pas d'injection (ciel dégagé) |

> **Pourquoi forcer un nuage ?** XP12 ne génère **pas** ses propres nuages via
> l'API `XPLMWeather` (contrairement à l'interface manuelle). Sans nuage source,
> aucune particule de pluie/neige n'apparaît. C'est la raison d'être de la
> plupart des dépendances ci-dessous.

---

## Tester une météo sans lancer de scénario (`injection_weather_test.py`)

Avant de lancer un batch complet (long, rend des images), tu peux **prévisualiser**
...
> Prérequis : un run doit déjà exister dans `runs/` (pour la pose), X-Plane doit
> tourner et le plugin `PI_weather.py` être actif. Utile pour itérer rapidement
> sur les paramètres météo (pluie, nuages, brouillard) avant de générer un batch.
...
> **Confirmation : oui, le type `3` (Cumulonimbus) donne la meilleure
> couverture pour la pluie.** C'est d'ailleurs le type que le code force
> automatiquement en mode « pluie seule », avec `cloud_coverage = 1.0`. Pour de
> la pluie fiable, `cloud_type = 3` est le choix recommandé.
